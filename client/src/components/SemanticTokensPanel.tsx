          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            ...
            <div className="p-4 border rounded-lg">
              ...
              >
                ...
                <div className="text-xs mb-3" style={{ color: 'rgba(0, 0, 0, .75)' }}>
    ...
    <tr className="border-b hover:bg-muted/10">
      ...
      <td className="p-3">
        <div className="flex items-center gap-2">
          ...
          <button
            className="h-6 w-6 p-0 opacity-50 hover:opacity-100 ml-auto"
            onClick={copyToClipboard}
          >
...
const PairingExample = ({
  ...
  return (
    <div>
      <div 
        className="px-3 py-2 rounded-md inline-flex items-center gap-2 mb-1"
        style={{ 
          backgroundColor: background,
          color: foreground,
          border: border ? `1px solid ${border}` : undefined
      ...
      <div className="text-xs flex gap-3 mt-1">
        <span className={`${ratio >= 4.5 ? 'text-green-500 font-medium' : 'text-red-500'}`}>
          {ratio.toFixed(2)}:1
        </span>
        <span className={`${isAALarge ? 'text-green-500' : 'text-red-500'}`}>AA Large</span>
        <span className={`${isAA ? 'text-green-500' : 'text-red-500'}`}>AA</span>
        <span className={`${isAAA ? 'text-green-500' : 'text-red-500'}`}>AAA</span>
      </div>
    </div>
  );
};